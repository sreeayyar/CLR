...
% This code produces a size table of CLR, M-CLR, H-LIML, J-AR and mKLM tests using
...
sh = 0.0025; % randomly select a subsample that is {0.25%, 0.5%, 1%, 2%} of the original sample size
select = random <= sh;

%% Run the following block to reset saved table files

%fid = fopen('empirical_sim_table.tex', 'w'); 
%fprintf(fid, '%s\n',' fraction, $N$ & $K$ & Avg. FF & Avg. $\widetilde{F}$ & $\frac{\mu^2}{\sqrt{K}\sqrt{\Upsilon}}$ & mKLM & Wald HLI & CLR & MCLR & jackknife AR');
%fprintf(fid, '%s\n', '\hline');
%fprintf(fid, '%s\n', '\hline');
%fclose(fid) ;
%% Create Instruments
...
p_mCLR = 0; p_CLR = 0; p_mKLM = 0; p_thli = 0;
...
parfor s = 1:sims
    ...
    X = 0.1*Xmean + v;
    Y = 0.1*X + Wcontrol + v + 0.1*epsilon;
    ...
    gamma_hat=pinv(Wcontrol'*Wcontrol)*Wcontrol'*(Y-X*beta);
    M_full=eye(n)-Z*pinv(Z'*Z)*Z';
    H1_full = Ybar'*M_full*Ybar;

    b0=[1;-beta];
    sigma_u0=b0'*H1_full*b0/(n-m-1);
    lam_tilde=((Y-X*beta)'*P2*(Y-X*beta))/((Y-X*beta)'*M_full*(Y-X*beta));
    Upsilon=P2*X; % (n*1)
    s_ee=(Y-X*beta)'*M_full*(Y-X*beta)/(n-m-1);
    s_ev=(Y-X*beta)'*M_full*X/(n-m-1);
    Y2_tilde=X-(Y-X*beta)*s_ev/s_ee;
    V_hat=M_full*Y2_tilde;
    kappa=sum(diag(P2).^2)/m;
    tau=m/n;
    Sigma_B=sigma_u0*(Y2_tilde'*P2*Y2_tilde+lam_tilde^2*Y2_tilde'*M_full*Y2_tilde);
    A_hat=(Upsilon'*(diag(P2)-tau))*(((Y-X*beta-Wcontrol*gamma_hat).^2)'*V_hat/n);
    B_hat=m*(kappa-tau)*(V_hat'*diag(((Y-X*beta-Wcontrol*gamma_hat).^2-sigma_u0))*V_hat)/(n*(1-2*tau+kappa*tau));
    Sigma_mKLM=Sigma_B+A_hat+A_hat'+B_hat;
    
    mKLM=(Y2_tilde'*P2*(Y-X*beta))*pinv(Sigma_mKLM)*(Y2_tilde'*P2*(Y-X*beta));
 ...
 %% Wald Test by Hausman et al (2012, QE)
    ...
    *Q_inv_h;
    ...
    t_hli = sqrt(n)*(beta1_HLI-beta)/sqrt((Psi_HLI(1,1)));
    ...
    %% Assemble Jaccknife AR stattistic components:
    ...
    lAR_stat_sims(s,:) = 1/sqrt(m)*lAR_func(beta);
    lAR_sigma_sims(s,:) = Sigma1_gg_func(beta);
end
 
%%

AR_rej = (lAR_stat_sims./sqrt(lAR_sigma_sims)) > JAK95;
p_mCLR = p_mCLR/sims;
p_CLR = p_CLR/sims;
p_mKLM = p_mKLM/sims;
p_thli = p_thli/sims;

%% Empirical Size Table

fid = fopen('empirical_sim_table.tex', 'a+'); 
fprintf(fid, '%4.3f%s & %4.0f  & %4.0f & %4.2f & %4.2f & %4.2f & %4.1f%s& %4.1f%s& %4.1f%s & %4.1f%s %4.1f%s\n',...
    mean(sh)*100,'\%',...
    n,m,mean(F_SY_sims),mean(Fhat_sims),Ftheo,...
    mean(p_mKLM)*100,'\%',...
    mean(p_thli)*100,'\%',...
    mean(p_CLR)*100,'\%',...
    mean(p_mCLR)*100,'\%', ...
